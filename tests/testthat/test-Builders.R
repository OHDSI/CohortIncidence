...
test_that("createTimeAtRiskDef works", {
  ...
  expect_equal(as.character(jsonlite::toJSON(t1)), '{"id":1,"start":{"dateField":"start","offset":0},"end":{"dateField":"end","offset":0}}')
  t2 <- CohortIncidence::createTimeAtRiskDef(id=1, startWith = "start", startOffset = 1); #setting start date
  expect_equal(as.character(jsonlite::toJSON(t2)), '{"id":1,"start":{"dateField":"start","offset":1},"end":{"dateField":"end","offset":0}}')
  t3 <-   CohortIncidence::createTimeAtRiskDef(id=1, startWith = "start", startOffset = 1, endWith = "start", endOffset = 30); #setting all dates
  expect_equal(as.character(jsonlite::toJSON(t3)), '{"id":1,"start":{"dateField":"start","offset":1},"end":{"dateField":"start","offset":30}}')
  
  # test validate
  expect_error({
    CohortIncidence::createTimeAtRiskDef(id=1, startWith = "xDate", startOffset = 1)
  }, "Invalid startWith option:xDate. Valid options are start, end")
  expect_error({
    CohortIncidence::createTimeAtRiskDef(id=1, endWith = "xDate", startOffset = 1)
  }, "Invalid endWith option:xDate. Valid options are start, end")
