...
public class Incidence_5_0_Test extends AbstractDatabaseTest {
	...
	@Test
	public void excludePersonTest() throws Exception {
		TestParams params = new TestParams();
		
		params.resultSchema = "person_exclude"; // this must be all lower case for DBUnit to work
		params.prepDataSets = new String[]{
			"/datasets/vocabulary.json",
			"/cohortincidence/timeAtRisk/personExclude_PREP.json"
		};
		params.designJson = ResourceHelper.GetResourceAsString("/cohortincidence/timeAtRisk/personExcludeTest.json");
		params.verifyDataSets = new String[]{"/cohortincidence/timeAtRisk/personExclude_VERIFY.json"};
		params.verifyCols = Arrays.asList(new String[]{COL_REF_ID, COL_TARGET_COHORT_ID, COL_TAR_ID, COL_SUBGROUP_ID, COL_OUTCOME_ID,
			COL_AGE_GROUP_ID, COL_GENDER_ID, COL_YEAR_ID,
			COL_PERSONS_PRE_EXCLUDE, COL_PERSONS_AT_RISK, COL_PERSONS_DAYS_PRE_EXCLUDE, COL_PERSON_DAYS,
			COL_PERSON_OUTCOMES_PRE_EXCLUDE, COL_PERSON_OUTCOMES, COL_OUTCOMES_PRE_EXCLUDE, COL_OUTCOMES,
	...
	/**
	 * Sets a TAR that will cause a person to be excluded from analysis.
   * The tar of start = start_date + 1d will lead to one person having their 
   * TAR pushed outside of observation period and not counted.
	 * @throws Exception 
	 */
	@Test
	public void invalidTarTest() throws Exception {
		TestParams params = new TestParams();
		
		params.resultSchema = "invalid_tar"; // this must be all lower case for DBUnit to work
		params.prepDataSets = new String[]{
			"/datasets/vocabulary.json",
			"/cohortincidence/timeAtRisk/invalidTar_PREP.json"
		};
		params.designJson = ResourceHelper.GetResourceAsString("/cohortincidence/timeAtRisk/invalidTarTest.json");
		params.verifyDataSets =  new String[]{"/cohortincidence/timeAtRisk/invalidTar_VERIFY.json"};
		params.verifyCols = Arrays.asList(new String[]{COL_REF_ID, COL_TARGET_COHORT_ID, COL_TAR_ID, COL_SUBGROUP_ID, COL_OUTCOME_ID,
							COL_PERSONS_PRE_EXCLUDE, COL_PERSONS_AT_RISK, COL_PERSONS_DAYS_PRE_EXCLUDE, COL_PERSON_DAYS,
							COL_PERSON_OUTCOMES_PRE_EXCLUDE, COL_PERSON_OUTCOMES, COL_OUTCOMES_PRE_EXCLUDE, COL_OUTCOMES,
							COL_INCIDENCE_PROPORTION_P100P, COL_INCIDENCE_RATE_P100PY});
	...
	/**
	 * Uses the firstAtRisk design to follow people to first outcome after TAR starts,
	 * ignoring prior outcomes that are not within clean window of TAR-start, and 
	 * stop follow up at first outcome.
	 * @throws Exception 
	 */
	@Test
	public void firstAtRiskTest() throws Exception {
		TestParams params = new TestParams();
		
		params.resultSchema = "first_at_risk"; // this must be all lower case for DBUnit to work
		params.prepDataSets = new String[]{
			"/datasets/vocabulary.json",
			"/cohortincidence/timeAtRisk/firstAtRisk_PREP.json"
		};
		params.designJson = ResourceHelper.GetResourceAsString("/cohortincidence/timeAtRisk/firstAtRiskTest.json");
		params.verifyDataSets =  new String[]{"/cohortincidence/timeAtRisk/firstAtRisk_VERIFY.json"};
		params.verifyCols = Arrays.asList(new String[]{COL_REF_ID, COL_TARGET_COHORT_ID, COL_TAR_ID, COL_SUBGROUP_ID, COL_OUTCOME_ID,
							COL_PERSONS_PRE_EXCLUDE, COL_PERSONS_AT_RISK, COL_PERSONS_DAYS_PRE_EXCLUDE, COL_PERSON_DAYS,
							COL_PERSON_OUTCOMES_PRE_EXCLUDE, COL_PERSON_OUTCOMES, COL_OUTCOMES_PRE_EXCLUDE, COL_OUTCOMES,
							COL_INCIDENCE_PROPORTION_P100P, COL_INCIDENCE_RATE_P100PY});
